...
# -------------------------------------------------------------------------------------
data = yf.download(tickers, period="4y")["Close"].tail(630)  # Extracted prices from yfinance
...
out_of_sample_data = data.tail(30)
...
plt.xlabel("Time")
plt.ylabel("Return")
plt.title(f"Back-tested Portfolio over Last 30 Days (Out-of-Sample)\nReturn = {(cumulative_growth[-1] - 1)*100:.2f}%")
plt.grid(True)
plt.show()
